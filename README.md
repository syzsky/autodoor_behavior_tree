...
| 🧩 **22 种内置节点**      | 6 种组合节点 + 6 种条件节点 + 10 种动作节点           |
...
| 📝 **脚本录制**          | TXT 脚本录制与回放                            |
| 📦 **PyInstaller打包** | PyInstaller 打包，内置 DD64.dll 虚拟键盘驱动      |
...
│   ├── resource_service.py    #   资源管理服务（收集/导入/缓存/清理）
│   ├── resource_importer.py   #   资源文件自动导入与分类
│   ├── path_resolver.py       #   相对路径与绝对路径转换
│   ├── project_manager.py     #   项目生命周期管理
│   ├── package_exporter.py    #   项目打包导出为 ZIP
