...
AutoDoor 行为树系统是一个功能完整的可视化行为树编辑与执行框架，专为 Windows 平台的自动化场景设计（应用辅助、RPA 流程等）。系统提供图形化编辑器、丰富的节点类型、脚本录制、OCR 识别、DD 虚拟键盘等能力，支持标准版与 DD 版两种打包模式。
...
| 🧩 **19 种内置节点** | 5 种复合节点 + 5 种条件节点 + 9 种动作节点            |
...
| DD版 | `autodoor_bt_dd.spec` | `autodoor-behaviortree-{ver}-dd`   | +DD64.dll, +hook_dd_input |
...
3. 可运行 `drivers/VC_redist.x64.exe` 安装运行时库
...
- **按键捕获取消功能**：按键节点的【修改】按钮支持再次点击取消
- **多屏显示器兼容性**：放大镜功能完全支持多显示器环境
- **Bug修复**：
  - 修复按键捕获时 Delete/Backspace 触发快捷键的问题
  - 修复 ESC 取消区域选择无效的问题

### v1.2.2a (2026-04-15)

- **随机节点**：新增随机节点，支持随机选择执行路径
- **OCR检测节点命名优化**：节点名称从"OCR检测"改为"文字检测"
- **鼠标移动参数逻辑优化**：相对移动时自动隐藏终点相关参数

### v1.2.0a (2026-04-14)

- **放大镜功能**：区域选择时显示放大镜，精确定位
- **随机范围功能**：时长参数支持随机变化
- **新增检查更新和使用文档按钮**

### v1.1.0 (2026-04-12)

- **OCR 引擎升级**：从 TesseractOCR 替换为 RapidOCR
  - 识别速度更快，准确率更高
  - 无需外部依赖，打包体积更小
  - 支持中英文混合识别
- **打包优化**：
  - 移除 tesseract 目录，减少打包体积
  - 排除 CUDA 相关 DLL，仅保留 CPU 版本
  - 固定 onnxruntime 版本为 1.19.0
- **功能增强**：
  - OCR 检测支持返回关键词中心位置
  - 添加 VC++ 运行时库检测与提示
  - 修复首次执行不检测的问题
